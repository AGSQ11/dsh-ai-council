...
export class AiCouncilService {
  ...
  async runCouncil(options = {}) {
    ...
    const createdAt = iso()
    if (!this.store.history(councilId)) this.store.beginHistory({ councilId, source, proposal, question, context, createdAt })
    ...
    try {
      ...
      this.store.patchHistory(councilId, {
        templateId: template.id, templateName: template.name, plannerReason: selection.plannerReason || '',
        selectedRoles: members.map(r => r.id), chairRoleId: chair.id,
        assignments: [...assignments.values()].map(a => ({ roleId: a.role.id, provider: a.route.provider, model: a.route.model })),
      })
      ...
      for (let round = 1; round <= cfg.maxRounds; round += 1) {
        ...
        this.store.patchHistory(councilId, { status: 'running', round, roundsTranscript: rounds })
      ...
      this.store.patchHistory(councilId, {
        ...result, status: 'ok', completedAt: iso(), proposal, question, context,
      ...
      this.store.patchHistory(councilId, { ...result, completedAt: iso(), proposal, question, context })
      return result
    }
  }
  ...
  routes() {
    ...
    return [
      { kind: 'exact', path: `${API_PREFIX}/state`, handler: (req, res) => { if (req.method !== 'GET') return json(res,405,{ok:false,error:'method-not-allowed'}); if (!guard(req,res)) return; json(res,200,{ok:true,...this.store.snapshot()}) } },
